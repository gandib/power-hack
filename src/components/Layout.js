...
const Layout = () => {
    ...
    useEffect(() => {
        const url = `http://localhost:5000/api/billing-list?page=${page}&size=${size}`;
        fetch(url, {
            method: 'GET',
            headers: {
                'content-type': 'application/json',
        ...
        <div>
            <div className="navbar px-12 bg-slate-100">
                <div className="flex-1">
                    <h1 className='font-semibold text-xl'>Power-Hack</h1>
                </div>
                <div className="flex-none">
                    <ul className="menu menu-horizontal px-1">
                        <li className='font-semibold text-xl'>Paid Total: {totalPaid}</li>
                    </ul>
                </div>
            </div>
